 ...
 * 用户可以在地图上自定义轨迹运势
 * 可以自定义路过某个点的图片，文字介绍等?
 * 主入口类昿<a href="symbols/BMapLib.LuShu.html">LuShu</a>＿
 * 基于Baidu Map API 1.2?.
 *
...
(function() {
     ...
     * @constructor
         * @param {Map} map Baidu map的实例对豿.
         * @param {Array} path 构成路线的point的数绿.
         * @param {Json Object} opts 可选的输入参数，非必填项。可输入选项包括＿<br />
         * {<br />"<b>landmarkPois</b>" : {Array} 要在覆盖物移动过程中，显示的特殊点。格式如丿:landmarkPois:[<br />
         *      {lng:116.314782,lat:39.913508,html:'加油竿',pauseTime:2},<br />
         *      {lng:116.315391,lat:39.964429,html:'高速公路收费站,pauseTime:3}]<br />
         * <br />"<b>icon</b>" : {Icon|String (图片路径)} 覆盖物的icon,
         * <br />"<b>iconWidth</b>" : {Num} 覆盖物宽度,
         * <br />"<b>iconHeight</b>" : {Num} 覆盖物高度,
         * <br />"<b>speed</b>" : {Number} 覆盖物移动速度，单位米/秿    <br />
         * <br />"<b>positionCallback</b>" : {function} 每移动一个点调用一次 <br />
         * <br />"<b>completeCallback</b>" : {function} 移动结束时调用<br />
         *  <br />"<b>endDisplay</b>" : {bool} 移动结束后是否继续显示<br />
         * <br />"<b>defaultContent</b>" : {String} 覆盖物中的内宿    <br />
         * }<br />.
         * @example <b>参考示例：</b><br />
     ...
     */
     BMapLib.LuShu = function(map, path, opts) {
        ...
        this._rotation = 0;//小车转动的角庿

         if(Object.prototype.toString.call(this._opts.icon) === "[object String]"){
             this._opts.icon=   new BMap.Icon(this._opts.icon, new BMap.Size(this._opts.iconWidth,this._opts.iconHeight));
         }
     ...
     * @param {Json Object} opts 用户输入的修改参敿.
     * @return 无返回倿.
     */
    ...
    LuShu.prototype.start = function() {
        var me = this,
            len = me._path.length;
        //不是第一次点击开姿,并且小车还没到达终点
        if (me.i && me.i < len - 1) {
            //没按pause再按start不做处理
            if (!me._fromPause) {
                return;
            }else if(!me._fromStop){
	            //按了pause按钮,并且再按start，直接移动到下一炿
	            //并且此过程中，没有按stop按钮
	            //防止先stop，再pause，然后连续不停的start的异帿
	            me._moveNext(++me.i);
            }
        }else {
            //第一次点击开始，或者点了stop之后点开姿
            me._addMarker();
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        /**
         * 计算两点间的距离
         * @param {Point} poi 经纬度坐标A炿.
         * @param {Point} poi 经纬度坐标B炿.
         * @return 无返回倿.
         */
        ...
        */
        ...
        _moveNext: function(index) {
            ...
            if (index < this._path.length - 1) {
                ...
                if(this._opts.positionCallback)this._opts.positionCallback(me._path[index + 1]);
            }
            else {
                if(this._opts.completeCallback)this._opts.completeCallback();
                this.stop();
            }
    ...
    /**
     * 自定义的overlay，显示在小车的上斿
     * @param {Point} Point 要定位的炿.
