...
window.BMAP_AUTHENTIC_KEY="M3itAma4bqeHa998OeCIPPTh";
window.BMAP_OFFLINE_URL=function(x,y,z){
	if(window.BMAP_OFFLINE_SERVER == 'http://online3.map.bdimg.com'){
		//http://online3.map.bdimg.com/tile/?qt=tile&x=M2&y=M1&z=9&styles=pl&scaler=1&udt=20150528
		return window.BMAP_OFFLINE_SERVER+'/tile/?qt=tile&x='+
		(x+'').replace(/-/gi, 'M')+'&y='+(y+'').replace(/-/gi, 'M')
		+'&z='+z+'&styles=pl&scaler=1&udt=20150528';
	}
	return window.BMAP_OFFLINE_SERVER+'/tile/'+z+'/'+(y+'').replace(/-/gi, 'M')+'/'+(x+'').replace(/-/gi, 'M')+'.png';
};
window.getOfflineMapUrl=function(c,d,b){//window.BaiduMapType && window.BaiduMapType == "SATELLITE"
	if(window.BMAP_OFFLINE_SERVER == 'http://online3.map.bdimg.com'){
		//http://shangetu3.map.bdimg.com/it/u=x=48555;y=19358;z=18;v=009;type=sate&fm=46&udt=20141015
		return 'http://shangetu3.map.bdimg.com/tile/?qt=tile&x='+
		(c+'').replace(/-/gi, 'M')+'&y='+(d+'').replace(/-/gi, 'M')
		+'&z='+b+'&styles=pl&scaler=1&udt=20150528';
		...
		return window.BMAP_OFFLINE_SERVER+'/wx/'+b+'/'+(c+'').replace(/-/gi, 'M')+'/'+(d+'').replace(/-/gi, 'M')+'.jpg';
		//return window.getOfflineMapUrl(c,d,b);
	}
};
(function(){
function aa(a) {
...
w.C.Xi = function (a, b) {
	...
	d = a.style[b];
	if (!d)
		var e = c.CA[b], d = a.currentStyle || (w.S.ba ? a.style : getComputedStyle(a, p)), d = e && e.get ? e.get(a, d) : d[e || b];
	if (e = c.gl)
		d = e.filter(b, d, "get");
	return d
};
...
w.oJ = function () {
	...
	a.prototype = {
		...
		move : function (a) {
			...
			if (10 < Math.abs((this.Yj ? a.touches[0].clientX : a.clientX) - this.bt) || 10 < Math.abs(c - this.ct))
				this.Vg = o
		},
		end : function (a) {
			pa(a);
...
w.extend(Ka.prototype, {
	...
	ja : u("ib"),
	my : function () {
		for (var a = this.H.PD; a < z.Iq.length; a++)
			z.Iq[a](this);
		this.H.PD = a
	},
	dE : function (a) {
...
z.ag = function (a, b) {
	...
	default:
		d = z.url.domain[a],
		"[object Array]" == Object.prototype.toString.call(d) ? (c = [], w.Wb.Db(d, function (a, d) {
				c[d] = z.url.proto + a + "/" + b
...
w.object.extend(I, {
		...
		if (d.ed == this.mj.jp)
			...
			if (d.ed == this.mj.YE) {
				...
				e.BB == q && (e.BB = o, setTimeout(function () {
						...
						if(0 != a.length){
							for(var i in a){
								Qb(window.BMAP_SERVER+'modules/'+a[i].split('_')[0]+'.js')
							}
						}else{
							e.qJ();
						}
	...
	JI : function (a) {
		if (a && this.JJ()[a])
			for (var a = this.JJ()[a], b = 0; b < a.length; b++)
				this.JI(a[b]), this.ud.Pk[a[b]] || this.IL(a[b])
	},
	IL : function (a) {
	...
	dX : function (a, b) {
		...
		c.ed = this.mj.jp;
		for (var e = 0, f = c.Vt.length; e < f; e++)
			c.Vt[e]();
		c.Vt.length = 0
	},
	CI : function (a, b) {
	...
	iT : function (a, b) {
		for (var c = this.ud.Xu, d = o, e = 0, f = c.length; e < f; e++)
			"" == c[e].yD && (c[e].dL == a ? c[e].yD = b : d = q);
		d && this.qJ()
	},
	qJ : function () {
		for (var a = this.ud.Xu, b = 0, c = a.length; b < c; b++)
	...
	removeItem : function (a) {
		if (a && "menuitem" == a.xb) {
			...
			for (c = this.nf.length; b < c; b++)
				this.nf[b] === a && (this.nf[b].remove(), this.nf.splice(b, 1), c--)
		}
	},
	PA : function () {
		this.na.push({
...
w.object.extend(eb.prototype, {
	...
	is : function (a) {
		if (!(a instanceof eb) || Math.max(a.ue, a.pe) < Math.min(this.ue, this.pe) || Math.min(a.ue, a.pe) > Math.max(this.ue, this.pe) || Math.max(a.te, a.oe) < Math.min(this.te, this.oe) || Math.min(a.te, a.oe) > Math.max(this.te, this.oe))
			return p;
	...
	extend : function (a) {
		if (a instanceof H) {
			...
			if (!this.ue || this.ue > b)
				this.hl.lng = this.ue = b;
			if (!this.pe || this.pe < b)
				this.Sk.lng = this.pe = b;
			if (!this.te || this.te > a)
				this.hl.lat = this.te = a;
			if (!this.oe || this.oe < a)
				this.Sk.lat = this.oe = a
		}
	},
	uE : function () {
...
z.Ee(function (a) {
	...
	a.addEventListener("clearoverlays", function () {
		...
		a = 0;
		for (var d = b.je.length; a < d; a++)
			b.je[a].Ri != q && (b.je[a].remove(), b.je[a] = p, b.je.splice(a, 1), a--, d--)
	});
...
w.extend(Bc.prototype, {
	...
	Tl : function (a, b, c) {
		...
		for (d = a.childNodes.length; l < d; l++)
			x = a.childNodes[l], x.kq = q, this.we.push(x);
		if (l = this.lm)
			for (var A in l)
				delete l[A];
		else
		...
		for (d = this.we.length; l < d; l++)
			n = this.we[l], n.kq || this.xe.push(n);
		this.Em = [];
		x = (e + c) * this.map.D.devicePixelRatio;
		l = 0;
		for (d = this.Bc.length; l < d; l++)
			A = this.Bc[l][0], i = this.Bc[l][1], t = A * e + g[0] - c / 2, v = (-1 - i) * e + g[1] - c / 2, y = k + "_" + b + "_" + A + "_" + i + "_" + f, m = this.lm[y], n = p, m ? (n = m.style, n.left = t + "px", n.top = v + "px", m.Ie || this.Em.push([A, i, m])) : (0 < this.xe.length ? (m = this.xe.shift(), m.getContext("2d").clearRect(-c / 2, -c / 2, x, x), n = m.style) : (m = document.createElement("canvas"), n = m.style, n.position = "absolute", n.width = e + c + "px", n.height = e + c + "px", this.Iw() && (n.WebkitTransform = "scale(1.001)"), m.setAttribute("width", x), m.setAttribute("height", x), a.appendChild(m)), m.id = y, n.left = t + "px", n.top = v + "px", -1 < y.indexOf("bg") && (t = "#F3F1EC", this.map.D.Nn && (t = this.map.D.Nn), n.background = t ? t : ""), this.Em.push([A, i, m])), m.style.visibility = "";
		l = 0;
		for (d = this.xe.length; l < d; l++)
			this.xe[l].style.visibility = "hidden";
		return this.Em
	},
	Iw : function () {
		...
		for (var b = this.fm.concat(this.Gf), c = b.length, d = 0; d < c; d++) {
			...
			if (e.zv) {
				...
				if (this.map.Mb()) {
					...
					f.style.display = "block", this.ve.style.display = "none", this.map.dispatchEvent(new L("vectorchanged"), {
						isvector : q
					})
			}
	...
	Dg : function (a) {
		...
		a = b.map.Mb();
		if (c instanceof cb)
			a && !c.am && (c.fa(this.map, this.ve), c.am = o);
	...
	$g : function (a) {
		...
		if (a instanceof cb)
			...
			if (a.xo)
				for (var b = 0, c = this.ng.length; b < c; b++)
					a == this.ng[b] && this.ng.splice(b, 1);
			else {
				b = 0;
			...
			for (var c = p, c = "2" == z.Kx ? [z.url.proto + z.url.domain.main_domain_cdn.other[0] + "/"] : [z.url.proto + z.url.domain.main_domain_cdn.baidu[0] + "/", z.url.proto + z.url.domain.main_domain_cdn.baidu[1] + "/", z.url.proto + z.url.domain.main_domain_cdn.baidu[2] + "/"], d = 0, e; e = this.fm[d]; d++)
				if (e.PX == o) {
					...
					e.getTilesUrl = function (b, d) {
						var e = b.x,
						k = b.y,
						l = "customimage/tile?&x=" + e + "&y=" + k + "&z=" + d + "&udt=20150401",
...
Tc.getTilesUrl = function (a, b, c) {
	...
	e = Sc[c];
	return window.BMAP_OFFLINE_URL(d,a,b);
...
w.extend(ld.prototype, {
	...
	ET : function () {
		...
		for (e = d; e <= b; e++)
			0 < e && (e == this.page ? a.push('<span style="margin-right:3px">' + e + "</span>") : 1 <= e && e <= this.ld && (d = '<span><a style="color:#7777cc;margin-right:3px" href="javascript:void(0)" onclick="{temp3}">[' + e + "]</a></span>", a.push(d.replace("{temp3}", "BMap.I('" + this.Q + "').toPage(" + e + ");"))));
...
w.extend(Na.prototype, {
	...
	tA : function () {
		if (this.B)
			for (var a = this.B.MC(), b = 0; b < a.length; b++)
	...
	RD : function (a) {
		for (var b = this.xg.length; b--; )
			this.xg[b] === a && this.xg.splice(b, 1)
	}
});
var Dd = Na.prototype;
U(Dd, {
...
we.prototype = {
	...
	DO : function (a) {
		if (a.wP === o)
			return this.LA = a.target, this.bw = Date.now(), o;
		if (this.LA && this.bw) {
			if (600 < Date.now() - this.bw)
				return this.bw = this.LA = p, o
		} else {
		...
		if ("select" != b && "input" != b && "textarea" != b)
			return a.stopImmediatePropagation ? a.stopImmediatePropagation() : a.SW = o, a.stopPropagation(), a.preventDefault(), this.bw = this.LA = p, q
	},
	Vu : function (a) {
		...
		if (this.enabled) {
			...
			if (this.options.zD && (this.options.sk ? (c = getComputedStyle(this.zb, p)[$d].replace(/[^0-9\-.,]/g, "").split(","), d =  + (c[12] || c[4]), c =  + (c[13] || c[5])) : (d = +getComputedStyle(this.zb, p).left.replace(/[^0-9-]/g, ""), c = +getComputedStyle(this.zb, p).top.replace(/[^0-9-]/g, "")), d != this.x || c != this.y))
				this.options.di ? this.Cd(se) : ue(this.TA), this.fj = [], this.Eq(d, c), this.options.Ww && this.options.Ww.call(this);
			this.mv = this.x;
			this.nv = this.y;
			this.bt = this.x;
			this.ct = this.y;
	...
	SQ : function (a) {
		...
		if (this.options.zoom && je && 1 < a.touches.length)
			...
			this.jg = b.pageY;
			if (0 < e || e < this.Od)
				e = this.options.jr ? this.x + c / 2 : 0 <= e || 0 <= this.Od ? 0 : this.Od;
			if (f > this.We || f < this.Vc)
				f = this.options.jr ? this.y + d / 2 : f >= this.We || 0 <= this.Vc ? this.We : this.Vc;
			this.JB += c;
			this.KB += d;
	...
	fu : function (a) {
		if (!(je && 0 !== a.touches.length)) {
			...
			if (b.uk)
				d = b.scale * b.yo, d = Math.max(b.options.tk, d), d = Math.min(b.options.hp, d), b.yo = d / b.scale, b.scale = d, b.x = b.Yw - b.Yw * b.yo + b.x, b.y = b.Zw - b.Zw * b.yo + b.y, b.zb.style[be] = "200ms", b.zb.style[$d] = "translate(" + b.x + "px," + b.y + "px) scale(" + b.scale + ")" + ve, b.uk = q, b.refresh(), b.options.Do && b.options.Do.call(b, a);
			else {
				if (b.Vg) {
					if (300 > i && b.options.zD) {
						...
						d = b.x + f.oa;
						e = b.y + g.oa;
						if (0 < b.x && 0 < d || b.x < b.Od && d < b.Od)
							f = {
								oa : 0,
								time : 0
							};
						if (b.y > b.We && e > b.We || b.y < b.Vc && e < b.Vc)
							g = {
								oa : 0,
								time : 0
							}
					...
					if (je)
						if (b.cJ && b.options.zoom)
							clearTimeout(b.cJ), b.cJ = p, b.options.Eo && b.options.Eo.call(b, a), b.zoom(b.ig, b.jg, 1 == b.scale ? b.options.XT : 1), b.options.Do && setTimeout(function () {
	...
	vn : function (a) {
		...
		if (b == this.x && c == this.y) {
			if (this.Vg && (this.Vg = q, this.options.Ww && this.options.Ww.call(this)), this.Yi && this.options.Aw && ("webkit" == Sd && (this.cK.style[ee] = "300ms"), this.cK.style.opacity = "0"), this.hj && this.options.Aw)
	...
	wS : function (a) {
		...
		if ("zoom" == b.options.aN) {
			if (d = b.scale * Math.pow(2, 1 / 3 * (d ? d / Math.abs(d) : 0)), d < b.options.tk && (d = b.options.tk), d > b.options.hp && (d = b.options.hp), d != b.scale)
				!b.Mx && b.options.Eo && b.options.Eo.call(b, a), b.Mx++, b.zoom(a.pageX, a.pageY, d, 400), setTimeout(function () {
					b.Mx--;
					!b.Mx && b.options.Do && b.options.Do.call(b, a)
				}, 400)
		} else
			c = b.x + c, d = b.y + d, 0 < c ? c = 0 : c < b.Od && (c = b.Od), d > b.We ? d = b.We : d < b.Vc && (d = b.Vc), 0 > b.Vc && b.scrollTo(c, d, 0)
	},
	sS : function (a) {
	...
	refresh : function () {
		...
		if ("string" == typeof this.options.Zs) {
			...
			for (b = c.length; a < b; a++)
				d = this.Bj(c[a]), d.left += this.CE, d.top += this.DE, this.Ce[a] = d.left < this.Od ? this.Od : d.left * this.scale, this.Ze[a] = d.top < this.Vc ? this.Vc : d.top * this.scale
		} else if (this.options.Zs) {
			for (this.Ce = []; d >= this.Od; )
				this.Ce[b] = d, d -= this.xt, b++;
			this.Od % this.xt && (this.Ce[this.Ce.length] = this.Od - this.Ce[this.Ce.length - 1] + this.Ce[this.Ce.length - 1]);
			b = d = 0;
			for (this.Ze = []; d >= this.Vc; )
				this.Ze[b] = d, d -= this.Jm, b++;
	...
	scrollTo : function (a, b, c, d) {
		...
		for (b = e.length; a < b; a++)
			e[a].TW && (e[a].x = this.x - e[a].x, e[a].y = this.y - e[a].y), this.fj.push({
				x : e[a].x,
				y : e[a].y,
				time : e[a].time || 0
			});
